...
| **invalid_scope** | `Invalid scope: update` | The app requested a scope that is not allowed by the Trusted Application. | Check the `Scope` field in the Trusted Application record. Only request scopes defined there (for example, `read update`). |
