...
No passwords are shared and every token is issued by @@name **Identity Server**.
...
No passwords are stored, and both the user and service are securely authenticated by the Identity Server.

```mermaid
flowchart LR
  David([🎸 David - User]) -->|Signs in| WebApp[💻 Web App]
  WebApp -->|Auth Request| IDP[🛡️ ERP.net Identity Server]
