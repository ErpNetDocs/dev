...
> [!NOTE]  
> All flows are handled by @@name **Identity Server**, which authenticates users, validates app identities, and issues tokens.
...
A user logs in through the @@name Identity Server, and your app receives an **access token** to act on behalf of that specific user.
...
It authenticates directly with the **Identity Server** using its credentials from the Trusted Application record.
...
- The **Identity Server** validates the app and issues tokens according to its configuration.  
...
Choose the flow that matches how your app operates - and let the @@name Identity Server handle authentication securely and consistently.
