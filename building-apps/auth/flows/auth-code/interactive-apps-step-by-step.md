...
This guide walks through the exact HTTP steps an interactive app takes to sign in a user with the @@name Identity Server, exchange the authorization code for tokens, and call the APIs.
...
Your app redirects the user to the instance @@name Identity Server. Example query:
...
After the user signs in successfully, the Identity Server redirects back to your registered `redirect_uri` with a short-lived authorization code:
