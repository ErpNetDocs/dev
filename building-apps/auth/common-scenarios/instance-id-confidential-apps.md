...
| ImpersonateAsCommunityUserAllowed | true or false | When `false`, only internal users may authenticate. When `true`, external (community) users may authenticate for identification, while API requests may use a service identity. |
...
| ApplicationSecretHash | `<base64(sha256(your-secret))>` | The hashed client secret used during the token request. |
