...
@@name follows the **OAuth 2.0** standard for token management, and all tokens are issued by the instance's **Identity Server**.
...
Tokens are issued by the @@name **Identity Server** after successful authentication.  
...
2. **Token Issuance** – @@name Identity Server issues the token representing the app or user identity.  
